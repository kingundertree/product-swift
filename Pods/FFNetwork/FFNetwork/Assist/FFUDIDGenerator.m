...
@interface FFUDIDGenerator ()
@property (nonatomic, strong) FFKeychain *myKeyChain;
...
@implementation FFUDIDGenerator

+ (FFUDIDGenerator *)sharedInstance
{
    static dispatch_once_t pred;
    static FFUDIDGenerator *sharedInstance = nil;
    dispatch_once(&pred, ^{
        sharedInstance = [[FFUDIDGenerator alloc] init];
    });
...
- (FFKeychain *)myKeyChain
{
    if (!_myKeyChain) {
        _myKeyChain = [[FFKeychain alloc] initWithService:serviceName withGroup:@"com.xiazer.music"];
    }
    return _myKeyChain;
}
