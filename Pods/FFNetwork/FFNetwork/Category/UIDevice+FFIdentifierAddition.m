//
//  UIDevice+FFIdentifierAddition.m
//  FFMusic
//
...
@implementation UIDevice (FFIdentifierAddition)
...
- (NSString *)FFNet_createUUID
{
    ...
    uuidWithDate = [[uuidWithDate substringToIndex:24] stringByAppendingString:date];
    return uuidWithDate;
}
...
- (NSString *)FFNet_uuid
{
    ...
    if (uuid.length == 0) {
        [[NSUserDefaults standardUserDefaults] setObject:[self FFNet_createUUID] forKey:key];
...
- (NSString *) FFNet_udid
{
    NSString *udid = [[FFUDIDGenerator sharedInstance] udid];
    if (udid.length==0) {
        udid = [self FFNet_uuid];
        [[FFUDIDGenerator sharedInstance] saveUDID:udid];
    }
    return udid;
}
...
- (NSString *)FFNet_macaddressMD5{
    ...
    if (macid.length == 0) {
        NSString *macaddress = [[UIDevice currentDevice] FFNet_macaddress];
